...
class CVPopupAlert:
    def __init__(self, detect_obj, score, image):
        ...
        self.window_name = f"ALERT: {self.detect_obj.upper()} DETECTED!"
        self.created = False
    ...
    def create(self):
        ...
        self.display_img = np.zeros((h + border, w, 3), dtype=np.uint8)
        self.display_img[border:, :] = self.image
        
        # Add red background for the alert
        cv2.rectangle(self.display_img, (0, 0), (w, border), (0, 0, 200), -1)
        
        # Add text with better visibility
        cv2.putText(self.display_img, alert_text, (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(self.display_img, confidence_text, (10, 55), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        # Create a named window that can be resized
        cv2.namedWindow(self.window_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(self.window_name, w, h + border)
        self.created = True
        
        # Save the detection image (smaller size)
        cv2.imwrite("detected_weapon.jpg", self.image)
        
        print(f"OpenCV popup alert prepared for: {self.detect_obj}")
...
def main():
    ...
    try:
        while True:
            ...
            for i in range(max_detections):
                ...
                if scores[0][i] * 100 > 75:
                    ...
                    if time.time() - detect_timer > notification_interval:
                        ...
                        alert = CVPopupAlert(object_name, score_percent, image)
                        alert.create()
                        alert.show()
    
                        # Mark it to be closed after a few seconds
                        alert_close_time = time.time() + 10  # Close after 10 seconds
                        active_alerts.append((alert, alert_close_time))
            ...
            remaining_alerts = []
            for alert, close_time in active_alerts:
                if current_time > close_time:
                    alert.close()
                else:
                    alert.show()  # Keep showing it
                    remaining_alerts.append((alert, close_time))
